...
un tableau contenant une ou plusieurs expressions régulières. Dans ce
dernier cas le lexème peut être réparti sur plusieurs
enregistrements. Ce peut être par exemple une chaîne de caractères
délimitée par des guillemets, des commentaires d'un programme C,
etc. Une telle spécification est surtout utile si les données à analyser
proviennent d'un flot de caractères.
